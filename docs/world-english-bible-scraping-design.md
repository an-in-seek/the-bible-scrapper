...
- 절 번호는 있고 각주 마커만 남아 텍스트가 비어 있음
  - WEB이 번역하지 않는 절이다. `(omitted)` 마커로 저장한다 (11.4 참고)
- 각주도 없이 텍스트가 비어 있음
  - 파싱 실패로 보고 해당 절 제외
...
- 적재 절 수: **31,102절** (실측)
  - 이 중 4절은 WEB이 번역하지 않아 `(omitted)` 마커로 저장된다(11.4 참고).
  - KJV와 총량이 같지만, **총 절 수만으로 완료를 판정하지 않는다.** 판정 기준은 11.4의 구조 검증 쿼리다.
...
**이 쿼리가 0 rows를 유지하려면 아래 4개 구절 처리가 전제된다.**

#### WEB이 번역하지 않는 4개 구절

WEB에는 아래 네 구절의 본문이 없다. 현대 역본에서 본문에서 빠지는 것으로 알려진 구절들이다.
...
**이 절들은 BibleGateway HTML에 스팬 자체가 없는 것이 아니다.** 절 스팬은 존재하지만 내용이 각주 마커뿐이다.
...
**저장 규칙: 절 번호를 건너뛰지 않고 `(omitted)` 마커로 저장한다.**

근거:

- 같은 DB의 개역개정이 동일한 구절을 `(없음)`, `없음`, `（없음）`으로 이미 표기하고 있다. 한글 성경 출판 관례이며, WEB만 구멍을 내면 역본 간 표현이 어긋난다.
- row가 없으면 **역본의 의도인지 스크래핑 실패인지 DB만 봐서 구분할 수 없다.** 마커가 있으면 "절 번호는 1부터 연속"이 불변식이 되어, 이후 어떤 구멍이든 곧바로 결함으로 판정된다.
- 각주 본문을 절 텍스트로 넣는 방식은 채택하지 않는다. "각주는 절 본문에 포함하지 않는다"(6.3)와 충돌하고, WEB이 번역하지 않기로 한 문장을 되살리는 결과가 된다.

마커 문자열은 영어 역본이므로 `(omitted)`를 쓴다. 한글 `(없음)`은 영어 본문에 섞이므로 쓰지 않는다.

#### 오탐 방지 장치

"빈 텍스트면 마커를 넣는다"로 구현하면 안 된다. 사이트 DOM이 바뀌어 모든 절이 빈 문자열이 되는 순간 31,000개의 `(omitted)` row가 조용히 쌓이고, 그 상황은 **시끄럽게 실패해야 한다.**

따라서 마커는 아래 조건을 모두 만족할 때만 기록한다.

- 절 스팬 안에 `sup.footnote`가 있다
- 인라인 마커(`sup.versenum`, `sup.footnote` 등) 제거 후 텍스트가 비어 있다

각주 없이 비어 있는 스팬은 파싱 실패로 보고 그대로 버린다.

한 절이 여러 조각으로 나뉘고 그중 일부만 마커인 경우에는, 번역된 조각만 이어붙이고 마커는 버린다.
...
5건 이상 나오거나 표에 없는 장이 나오면 파서가 정상 본문을 마커로 바꾸고 있다는 뜻이므로 즉시 조사한다.
...
- 검증 4에서 구멍이 발견된 특정 장만 다시 채울 수도 있다.
...
- 검증 4가 1건이라도 나옴 → 실제 누락이므로 조사 후 해당 장 재적재
- `(omitted)` 마커가 4건을 넘거나 11.4 표에 없는 장에서 나옴 → 파서가 정상 본문을 마커로 바꾸는 중이므로 즉시 중단
...
12. 11.4의 검증 쿼리 1~6과 마커 row 검증 전부 통과 확인
