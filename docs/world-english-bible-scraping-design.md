...
부분 실행(`--start-book`, `--start-chapter`)은 어디까지나 분할 실행과 검증 수단이며, 완료 기준은 [11. 전권 적재 운영 계획](#11-전권-적재-운영-계획)의 검증 쿼리를 모두 통과하는 상태다.
...
- 예상 적재 절 수: KJV 기준 약 31,100절
  - WEB은 일부 구절 분절 방식이 KJV와 달라 총량이 정확히 일치하지 않을 수 있다. 따라서 **총 절 수를 완료 판정 기준으로 쓰지 않는다.** 판정 기준은 11.4의 구조 검증 쿼리다.
